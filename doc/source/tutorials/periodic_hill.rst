...
.. Warning::
  ...
  This tutorial is written for the most recent version of *NekRS* available on GitHub.`.
...
Cases are setup in NekRS by editing case files. Users should select an editor of choice with which to do this (e.g vi). A case being simulated involves data for mesh, parameters, etc. As a first step, the user should create a case directory in their run directory.

.. code-block:: console

  $ cd $NEKRS_HOME 
  $ mkdir -p run/periodic_hill
  $ cd run/periodic_hill
...
.. literalinclude:: periodic_hill/hillp.usr
   :language: fortran
   :lines: 131-154
   :emphasize-lines: 20
...
.. literalinclude:: periodic_hill/SIZE
   :language: fortran
   :lines: 10 - 21
